        ...
        <div className="grid md:grid-cols-2 gap-6 items-start">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }}
            ...
            <div className="grid grid-cols-[1fr_80px_80px] text-center text-[10px] uppercase tracking-wider font-bold border-b border-gray-100">
              ...
              <div className="p-3 text-[#1A56DB] bg-blue-50/50">IC-ETSS</div>
            </div>
            {features.map((f, i) => (
              <motion.div key={i}
                initial={{ opacity: 0 }}
                ...
                <div className="p-3 text-left text-gray-700 text-sm font-medium pl-5">{f.name}</div>
                <div className="p-3 flex justify-center items-center"><X className="w-4 h-4 text-red-300" /></div>
                <div className="p-3 flex justify-center items-center bg-blue-50/30"><Check className="w-4 h-4 text-[#059669]" /></div>
              </motion.div>
            ))}
          </motion.div>
          ...
          <div className="space-y-3">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.4 }}
