    ...
    <section className="section-padding gradient-section" ref={ref}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
          {benefits.map((b, i) => (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.95 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.4, delay: 0.1 + i * 0.08 }}
              ...
              <span className="font-semibold text-foreground leading-snug">{b.title}</span>
            </motion.div>
          ))}
        </div>

        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.6, delay: 0.6 }}
          className="rounded-2xl p-6 border-l-4 border-l-primary bg-emerald-light">
          <p className="text-foreground/80 leading-relaxed">
            <span className="text-primary font-semibold">DOI</span> ensures your research is citable,
            permanently accessible, and globally discoverable — making your contribution part of the world's scholarly record.
          </p>
        </motion.div>
      </div>
    </section>
