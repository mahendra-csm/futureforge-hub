...
const features = [
  "Indexed research ecosystem", "DOI-backed documentation", "CPD accreditation",
  "Metadata exposure (170M+ records)", "Global academic collaboration network", "Transparent professional recognition",
];
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }}
          ...
          {features.map((f, i) => (
            <div key={i} className="grid grid-cols-3 text-center border-b border-border/50 last:border-0">
              <div className="p-4 text-left text-foreground font-medium text-sm">{f}</div>
              <div className="p-4 flex justify-center items-center"><X className="w-5 h-5 text-destructive/50" /></div>
              <div className="p-4 flex justify-center items-center"><Check className="w-5 h-5 text-primary" /></div>
            </div>
          ))}
        </motion.div>
      </div>
